...
if "parent_id" in st.query_params.keys():
    ...
    st.session_state["parent_info"] = temp_client.datasets.get_remote_dataset(
        dataset_id=st.query_params["parent_id"]
    )
    del temp_client
if "dataset_id" in st.query_params.keys():
    if st.query_params["dataset_id"] == "new":
        ...
        dmode = "Edit"
        temp_client = P2F_Client(hostname="localhost", port=8000, https=False)
        st.session_state["existing_data"] = temp_client.datasets.get_remote_dataset(
            dataset_id=st.query_params["dataset_id"]
        )

st.set_page_config(page_title=f"{dmode} dataset", page_icon="➕", layout="wide")
...
def get_url_form_data():
    ...
    if url_form_furl.host == "doi.org":
        ...
        request_furl = (
            request_furl
            / url_form_furl.path.segments[-2]
            / url_form_furl.path.segments[-1]
        )
        intermediate_request = requests.get(request_furl)
        if intermediate_request.ok:
            intermediate_json = intermediate_request.json()
            try:
                intermediate_dataset_title = intermediate_json["data"]["attributes"][
                    "titles"
                ][0]["title"]
            except:
                pass
            try:
                intermediate_dates = {
                    x["dateType"]: date2datetime(x["date"])
                    for x in intermediate_json["data"]["attributes"]["dates"]
                }
...
def add_sub_dataset():
    st.session_state["subdataset_list"].append(
        st.session_state[
            f"main_form_subdataset_{st.session_state['subdataset_counter']:03}"
        ]
    )
...
with st.form(key="url_form"):
    ...
    st.text_input("What is the dataset URL?", key="url_form_url")
...
if st.session_state["FormSubmitter:url_form-Submit"]:
    with st.form(key="secondary_form"):
        ...
        with st.container():
            st.write(
                "Please select the ages/epochs/time slices contained within the dataset"
            )
        ...
        st.pills(
            "Was this dataset created by the P2F project?",
            options=["Yes", "No"],
            key="main_form_original_p2f",
        )
        # Does this dataset have sub-datasets?
        subdatasets = st.pills(
            "Is this dataset a sub dataset?",
